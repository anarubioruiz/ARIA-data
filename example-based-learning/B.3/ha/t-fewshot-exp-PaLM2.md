...
### RULES
```yaml
- description: "Alternative lighting - Turn on music room light when occupancy detected at den and DenMusicRoomDoor is open"
trigger:
platform: state
...
state: "on"
- condition: state
entity_id: binary_sensor.DenMusicRoomDoor
state: "on"
action:
service: light.turn_on
entity_id: light.MusicRoomLight
...
state: "on"
- condition: state
entity_id: binary_sensor.MusicRoomOccupancy
state: "off"
action:
service: light.turn_off
entity_id: light.MusicRoomLight
...
### EXPLANATION
The rules illuminate the den with fixtures that are not the primary ones. The first rule turns on the music room light when occupancy is detected at the den and the door is open (the light can not reach the den if it can not pass through the door). The second rule turns off the music room light when no occupancy is detected at the den and no occupancy is detected at the music room.
## END
