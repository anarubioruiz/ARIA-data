...
### RULES
```
- description: "Turn on the studio light when occupancy is detected and the light level is low"
trigger:
- platform: state
entity_id: binary_sensor.studio_occupancy_detector
to: "on"
- platform: numeric_state
entity_id: sensor.studio_light_level
below: 100
action:
service: light.turn_on
entity_id: light.studio_light_bulb
...
service: light.turn_off
entity_id: light.studio_light_bulb
- description: "Open the curtains when occupancy is detected and the light level is high"
trigger:
- platform: state
entity_id: binary_sensor.studio_occupancy_detector
to: "on"
...
- description: "Close the curtains when occupancy is not detected"
trigger:
platform: state
entity_id: binary_sensor.studio_occupancy_detector
to: "off"
...
### EXPLANATION
The first rule turns on the studio light when occupancy is detected and the light level is low. The second rule turns off the studio light when occupancy is not detected. The third rule opens the curtains when occupancy is detected and the light level is high. The fourth rule closes the curtains when occupancy is not detected.
## END
