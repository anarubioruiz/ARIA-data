...
### RULES
```
- description: "Turn on the light when someone is in the living room"
trigger:
- platform: state
...
service: light.turn_on
entity_id: light.sb1
- description: "Turn off the light when no one is in the living room"
trigger:
- platform: state
entity_id: binary_sensor.os1
to: "off"
...
### EXPLANATION
The first rule turns on the light when occupancy is detected in the living room. The second rule turns off the light when there is no occupancy.
## END
