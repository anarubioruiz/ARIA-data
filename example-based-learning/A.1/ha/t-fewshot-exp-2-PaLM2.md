...
### RULES
```
- description: "Turn on the light when it's dark and someone is in the kitchen"
trigger:
- platform: state
...
- description: "Turn off the light when no one is in the kitchen"
trigger:
- platform: state
entity_id: binary_sensor.kitchen_occupancy
to: "off"
...
### EXPLANATION
The first rule turns on the light when occupancy is detected and the lighting is insufficient. The second rule turns off the light when there is no occupancy.
## END
